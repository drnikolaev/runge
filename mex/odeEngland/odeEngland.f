...
C     Check for proper number of arguments. 
      IF (nrhs .ne. 9) THEN
         CALL mexErrMsgTxt('9 inputs required but ' // 
     1    TRIM(TOSTR4(nrhs)) //
...
C     Check inputs 
     ...
     1      'Input #1 (''F'') length must be less than 256.')
      ...
      CALL mxCopyPtrToReal8(mxGetPr(prhs(4)),  X,    M)
...
30    CALL SENGL(PC, F,
     1           M, Hmin, Hmax, EPS, P, X, T0, H, R, IERR)
      ...
      IF (IERR .EQ. 0) THEN
          GOTO 20
      ELSE
          CALL mexErrMsgTxt('odeEngland failed to converge')
      ENDIF
40    CONTINUE
