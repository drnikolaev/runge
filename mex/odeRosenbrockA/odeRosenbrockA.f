...
CDEC$ ENDIF
      ...
      INTEGER lF(1), rF(1), lFJ(1), rFJ(1)
      CHARACTER*256 sF, sFJ
      CHARACTER*(2) TOSTR, TOSTR4
      COMMON /odeRosABLOCK/ lF, rF, lFJ, rFJ, sF, sFJ
      INTEGER PC
      EXTERNAL F, FJ
...
C     Check inputs 
     ...
     1      'Input #2 (''FJ'') length must be less than 256.')
      ...
      rF(1)   = mxCreateDoubleMatrix(M, 1, 0)   ! X
      rFJ(1)  = mxCreateDoubleMatrix(M, 1, 0)   ! X

      ALLOCATE (X(M))
      ALLOCATE (R(6*M*M + 4*M))
      R = 0.D0
...
25    H1 = T - T0
      ...
      H = H1
      BUL = .TRUE.
30    CALL SROSA (PC, F, FJ,
     1            M, Hmin, Hmax, EPS, P, X, T0, H, R, IERR)
      IF (BUL .AND. DABS(H1) .GT. DABS(H)) BUL = .FALSE.
      IF (IERR .EQ. 0) THEN
          GOTO 20
      ELSE
          CALL mexErrMsgTxt('odeRosenbrockA failed to converge')
      ENDIF
40    CONTINUE
      ...
      CALL mxCopyReal8ToPtr(X, mxGetPr(rF(1)), M)

      CALL mexCallMATLAB(1, lF, 1, rF, sF)
     ...
     2        TRIM(TOSTR(M)))
      ...
      CALL mxCopyReal8ToPtr(X, mxGetPr(rFJ(1)), M)

      CALL mexCallMATLAB (1, lFJ, 1, rFJ, sFJ)
