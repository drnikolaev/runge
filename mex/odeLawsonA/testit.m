...
tStart=tic;
for i = 1:N
    [xt,H] = odeLawsonA('A', 'fi', Ts(i), Ts(i+1), xt, H, Hmin, Hmax, eps, P);
    YL(:,i+1) = xt;
end
tElapsed=toc(tStart);
fprintf('odeLawsonA\t%0.5f sec\n', tElapsed)
...
title('odeLawsonA solution');
hold on;


xt = x;
YO(:,1) = xt;
options = odeset('AbsTol', 1.e-10, 'RelTol', 1.e-13, 'InitialStep', H, 'MaxStep', Hmax, 'Jacobian', @rightJ);
tStart=tic;
for i = 1:N
